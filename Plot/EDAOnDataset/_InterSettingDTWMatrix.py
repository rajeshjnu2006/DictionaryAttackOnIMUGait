...
imitators = ['Imitator4','Imitator5','Imitator6','Imitator7','Imitator8','Imitator9']
imitators = ['Imitator9']
...
for imt in imitators:  ## Traversing over all imitators
    ...
    for gcat in list_of_gcats:
        ...
        if gcat == 'speed':
            ...
            if imt in ['Imitator4','Imitator6']:
                list_of_settings = ['slength_short', 'slength_normal', 'slength_long']
            else:
                list_of_settings = ['slength_short', 'slength_normal', 'slength_long', 'slength_longer']
        elif gcat == 'swidth':
            if imt in ['Imitator4','Imitator6']:
                list_of_settings = ['swidth_close', 'swidth_normal', 'swidth_wide']
            else:
                list_of_settings = ['swidth_close', 'swidth_normal', 'swidth_wide', 'swidth_wider']
        else:
            if imt in ['Imitator4','Imitator6']:
                list_of_settings = ['tlift_back', 'tlift_normal', 'tlift_front']
            else:
        ...
        plt.figure(imt+gcat+'dtw')
        sns.heatmap(df_dtw_dist,cmap='BuGn_r', annot=True,fmt='.1f',xticklabels=df_dtw_dist.columns.values,yticklabels=df_dtw_dist.columns.values)
        plt.title(imt+": DTW distance between different settings of "+gcat)
        plt.tight_layout()
    plt.show()