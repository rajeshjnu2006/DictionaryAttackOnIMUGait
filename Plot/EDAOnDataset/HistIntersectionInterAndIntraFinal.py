### IF YOU ARE GETTING AN ERROR CHECk THE FOLDER NAMES INSIDE THE FOLDERS>..

### I CHANGED IT FOR IJCB 2023 for I1 and I7 and didnt have time to change it for other folders...

import numpy as np
# ref: https://mpatacchiola.github.io/blog/2016/11/12/the-simplest-classifier-histogram-intersection.html
...
imdb_path = os.path.join(os.path.dirname(os.path.dirname(os.getcwd())), 'Storage','PreprocessedProfileAnonymized')
imitators = os.listdir(imdb_path)
# exclude_imt_list = ['I4', 'I6']
exclude_imt_list = []
...
total_points = 2070
num_frames_to_compare = 8
list_of_gcats = ['SP', 'SW', 'TL','SP']
imitators = ['I4','I5','I6','I7','I8','I9']
imitators = ['I7']
...
for imt in imitators:  ## Traversing over all imitators
    ...
    for gcat in list_of_gcats:
        ...
        if gcat == 'SP':
            ...
            if imt in ['I4','I6']:
                list_of_settings = ['SL_short', 'SL_normal', 'SL_long']
            else:
                list_of_settings = ['SL_short', 'SL_normal', 'SL_long', 'SL_longer']
        elif gcat == 'SW':
            if imt in ['I4','I6']:
                list_of_settings = ['SW_close', 'SW_normal', 'SW_wide']
            else:
                list_of_settings = ['SW_close', 'SW_normal', 'SW_wide', 'SW_wider']
        else:
            if imt in ['I4','I6']:
                list_of_settings = ['TL_back', 'TL_normal', 'TL_front']
            else:
                list_of_settings = ['TL_back', 'TL_normal', 'TL_front', 'TL_up']
        #############################
        ...
        for first in list_of_settings:
            ...
            first_raw_data = pd.read_csv(first_path, sep=',', header=None, names=DefaultParam.NEW_COLUMN_NAMES)
            ...
            # Smoothing
            firstX = Prep.smooth(firstX, window_len=int(samplerate * 5 / 100))
            firstY = Prep.smooth(firstY, window_len=int(samplerate * 5 / 100))
            firstZ = Prep.smooth(firstZ, window_len=int(samplerate * 5 / 100))
            ...
            for second in list_of_settings:
                ...
                # Computing the feature vector for the current imitator at current setting
                second_raw_data = pd.read_csv(second_path, sep=',', header=None, names=DefaultParam.NEW_COLUMN_NAMES)
                ...
                # Smoothing
                secondX = Prep.smooth(secondX, window_len=int(samplerate * 5 / 100))
                secondY = Prep.smooth(secondY, window_len=int(samplerate * 5 / 100))
                secondZ = Prep.smooth(secondZ, window_len=int(samplerate * 5 / 100))
                # Sliding window based cutting of the signal
                # ParameterskNN.WINSTEP = 4
                ...
                for ind1, frameX1 in enumerate(chosenFirstX):
                    for ind2, frameX2 in enumerate(chosenSecondX):
                        if first == second and ind1==ind2:
                            pass
                            # print('first, second and ind1, ind2:', first, second, ind1, ind2)
                            # print('Skipping the comparision with the same frame in the same setting')
                        else:
                            hist1, _ = np.histogram(frameX1, bins=num_bins, range=bin_range)
                            hist2, _ = np.histogram(frameX2, bins=num_bins, range=bin_range)
                            distance = return_intersection(hist1, hist2)
                            dist_list.append(distance)
                # print("{}: Average DTW dist b/w frames of {} and {} is {}".format(imt, first, second,int(stat.mean(dist_list))))
                # print(f'{ind1} and {ind2}')
                avg_dist_list.append(stat.mean(dist_list))
            # print('avg_dist_list',avg_dist_list)
            df_dtw_dist.loc[first] = avg_dist_list
        print(df_dtw_dist.to_string())
        # fig, ax = plt.subplots(1, 1,figsize=(4, 3))
        # plt.figure(imt+gcat+'hist_intersection')
        cmapp = sns.cubehelix_palette(8)
        # plt.title(imt+":"+gcat) # NOt necessary
        subfig_counter=subfig_counter+1 # cmap= YlGnBu below
        chart = sns.heatmap(df_dtw_dist,cmap='Blues', annot=True, ax= axes[subfig_counter],fmt='.3f',xticklabels=df_dtw_dist.columns.values, yticklabels=df_dtw_dist.columns.values, vmin=0.5, vmax=1, linewidths=.2, annot_kws={"size": 10})
        # chart.set_xticklabels(chart.get_xticklabels(), rotation=25)
        # chart.set_yticklabels(chart.get_yticklabels(), rotation=45)
        # plt.title(imt+": intersection between histograms of "+gcat)
        plt.tight_layout()
plt.show()