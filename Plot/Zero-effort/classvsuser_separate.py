...
p4 = sns.heatmap(ZeroEffortFAR, annot=True, cmap=cm, ax=ax[0], annot_kws={"size":8}, vmin=0, vmax=100, fmt='.0f',linewidths=0.1, cbar=True, cbar_kws = dict(use_gridspec=False,location="top"))
plt.xlabel("Users")
plt.ylabel("Classifiers")
ax[0].set_title('FAR under zero-effort attack for '+str(sensor_name_list[0]))

p5 = sns.heatmap(ZeroEffortFRR, annot=True, cmap=cm, ax=ax[1], annot_kws={"size":8}, vmin=0, vmax=100, fmt='.0f',linewidths=0.1, cbar=False)
plt.xlabel("Users")
plt.ylabel("Classifiers")
ax[1].set_title('FRR under zero-effort attack for '+str(sensor_name_list[0]))

p6 = sns.heatmap(ZeroEffortHTER, annot=True, cmap=cm, ax=ax[2], annot_kws={"size":8}, vmin=0, vmax=100, fmt='.0f',linewidths=0.1, cbar=False)
plt.xlabel("Users")
plt.ylabel("Classifiers")
plt.xticks(np.linspace(0.5,56.5,56, endpoint=False),user_list+['Average'])
ax[2].set_title('HTER under zero-effort attack for '+str(sensor_name_list[0]))
