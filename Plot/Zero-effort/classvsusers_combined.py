...
fig, ax = plt.subplots(6, 1, sharex='col', sharey='row',figsize=(10, 10))
...
ax[0].set_title('FAR under zero-effort attack for '+str(sensor_name_list[0]))

p2 = sns.heatmap(ZeroEffortFRR, annot=True, cmap=cm, ax=ax[1], annot_kws={"size":8}, vmin=0, vmax=100, fmt='.0f',linewidths=0.1, cbar=False)
plt.xlabel("Users")
plt.ylabel("Classifiers")
ax[1].set_title('FRR under zero-effort attack for '+str(sensor_name_list[0]))
p3 = sns.heatmap(ZeroEffortHTER, annot=True, cmap=cm, ax=ax[2], annot_kws={"size":8}, vmin=0, vmax=100, fmt='.0f',linewidths=0.1, cbar=False)
plt.xlabel("Users")
plt.ylabel("Classifiers")
plt.xticks(np.linspace(0.5,56.5,56, endpoint=False),user_list+['Average'])
ax[2].set_title('HTER under zero-effort attack for '+str(sensor_name_list[0]))
...
print(ZeroEffortFRR.to_string())
print(ZeroEffortHTER.to_string())

# plt.figure(2)

p4 = sns.heatmap(ZeroEffortFAR, annot=True, cmap=cm, ax=ax[3], annot_kws={"size":8}, vmin=0, vmax=100, fmt='.0f',linewidths=0.1, cbar=False)#, cbar_kws = dict(use_gridspec=False,location="top"))
plt.xlabel("Users")
plt.ylabel("Classifiers")
ax[3].set_title('FAR under zero-effort attack for '+str(sensor_name_list[0]))

p5 = sns.heatmap(ZeroEffortFRR, annot=True, cmap=cm, ax=ax[4], annot_kws={"size":8}, vmin=0, vmax=100, fmt='.0f',linewidths=0.1, cbar=False)
plt.xlabel("Users")
plt.ylabel("Classifiers")
ax[4].set_title('FRR under zero-effort attack for '+str(sensor_name_list[0]))

p6 = sns.heatmap(ZeroEffortHTER, annot=True, cmap=cm, ax=ax[5], annot_kws={"size":8}, vmin=0, vmax=100, fmt='.0f',linewidths=0.1, cbar=False)
plt.xlabel("Users")
plt.ylabel("Classifiers")
plt.xticks(np.linspace(0.5,56.5,56, endpoint=False),user_list+['Average'])
ax[5].set_title('HTER under zero-effort attack for '+str(sensor_name_list[0]))
plt.subplots_adjust(left=0.08, bottom=0.17, right=.98, top=0.80, wspace=0.20, hspace=0.20)
