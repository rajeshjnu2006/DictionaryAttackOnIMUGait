...
########################################################################################################################
ZeroEffort = pd.DataFrame(columns=['user', 'sensors', 'far', 'classifier'])
DictionaryEffort = pd.DataFrame(columns=['user', 'sensors', 'far', 'classifier'])
HighEffort = pd.DataFrame(columns=['user', 'sensors', 'far', 'classifier'])
...
for cls in classifiers:
    ...
    #
            ZeroEffort.loc[zcounter] = [ze_best.user, sensor_dict[sensor_comb], ze_best.far, classifiers[cls]]
            zcounter = zcounter+1

            DictionaryEffort.loc[dcounter] = [de_best.user, sensor_dict[sensor_comb], de_best.far, classifiers[cls]]
            dcounter = dcounter+1
            if user in DefaultParam.HIGHEFFORT_USER_LIST:
                ...
                HighEffort.loc[hcounter] = [he_best.user, sensor_dict[sensor_comb], he_best.far, classifiers[cls]]
                hcounter = hcounter+1

print(ZeroEffort)
print(DictionaryEffort)
print(HighEffort)
########################################################################################################################
sns.set()
