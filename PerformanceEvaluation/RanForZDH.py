...
ulptable = pd.DataFrame(columns=['user', 'attacktype', 'attacker', 'sensors', 'nfeatures', 'n_estimators',
                                 'max_features', 'max_depth', 'min_samples_leaf', 'far', 'frr', 'hter','refhter'])
row_counter = 0
########################################################################################################################
...
for user in gen_user_list:
    ...
    for curr_comb in sensor_configurations:
        ...
        for num_features in list_num_features:
            ...
            # First create the base model to tune
            CUAuthModel = RandomForestClassifier(n_jobs=-1,random_state=DefaultParam.RANDOM_SEED)
            SearchTheBestParam = GridSearchCV(estimator=CUAuthModel, param_grid=param_grid, cv=10, n_jobs=-1,verbose=0, scoring=scorerHTER)
            SearchTheBestParam.fit(training_data, training_labels)

            n_estimators = SearchTheBestParam.best_params_['n_estimators']
            max_features = SearchTheBestParam.best_params_['max_features']
            max_depth = SearchTheBestParam.best_params_['max_depth']
            min_samples_leaf = SearchTheBestParam.best_params_['min_samples_leaf']

            ############################################################################################################
            ########### Validation
            ############################################################################################################
            # Retraining the model again and testing under different attack scenarios
            FinalModel = RandomForestClassifier(n_estimators=n_estimators, max_depth=max_depth,
                                            max_features=max_features, max_leaf_nodes=min_samples_leaf, n_jobs=-1,random_state=DefaultParam.RANDOM_SEED)
            ...
            ulptable.loc[row_counter] = [user, "zero-effort", "rest", curr_comb, num_features, n_estimators,
            ...
            for dict_attacker in dict_attacker_list:
                ...
                row_counter = row_counter + 1
                ulptable.loc[row_counter] = [user, "dict-effort", dict_attacker, curr_comb, num_features, n_estimators,
                                             max_features, max_depth, min_samples_leaf,
            ...
            if user in DefaultParam.HIGHEFFORT_USER_LIST:  # Running only for the first 18 users
                for attempt in attempt_list:
                    ...
                    # updating the giant perf table
                    row_counter = row_counter + 1
                    ulptable.loc[row_counter] = [user, "high-effort", attempt, curr_comb, num_features,
                                                 n_estimators,max_features, max_depth, min_samples_leaf,
                                                 tsh_far, tsh_frr, tsh_hter, tsz_hter]
            print('n_estimators, max_features, max_depth, min_samples_leaf',n_estimators, max_features, max_depth, min_samples_leaf)
