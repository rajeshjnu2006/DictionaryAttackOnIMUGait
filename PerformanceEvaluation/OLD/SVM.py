...
ulptable = pd.DataFrame(
    columns=['User', 'Sensors', 'Num_features', 'CVal', 'Gamma', 'Kernel', 'Training_score', 'FAR', 'FRR',
             'HTER'])
...
#""https://medium.com/all-things-ai/in-depth-parameter-tuning-for-svc-758215394769""
    # Create a dictionary of ParametersRandFor
            ...
            SVMBasedModel = svm.SVC(random_state=DefaultParam.RANDOM_SEED, tol= 1e-5)
            ...
            # ,tol=1e-5 is important to control the randomization effect .. read sklearn docs
            ...
            gamma = SVMGSearch.best_params_['gamma']
            kernel = SVMGSearch.best_params_['kernel']
            Training_stats.append([fs_threshold, cval, gamma, kernel, SVMGSearch.best_score_])
        ...
        FinalModel = svm.SVC(C=best_cval, gamma=best_gamma,kernel=best_kernel, random_state=DefaultParam.RANDOM_SEED,tol=1e-5)
        ...
        # https://pyformat.info/
        print(
            f'{user} with sensor_comb:{curr_comb}, num_features: {best_fstheshold}, CVal:{best_cval}, Gamma:{best_gamma}, Kernel: {best_kernel},training score:{best_training_score},far:{far:.3f}, frr:{frr:0.3f} hter:{hter:0.3f}')
        ulptable.loc[row_counter] = [user, curr_comb, best_fstheshold, best_cval, best_gamma, best_kernel,
...
best_perf_table = pd.DataFrame(
    columns=['User', 'Sensors', 'Num_features', 'CVal', 'Degree', 'Gamma', 'Kernel', 'Training_score', 'FAR', 'FRR',
             'HTER'])
user_counter = 0
for user in ordered_genuine_users:
    cuser_df = ulptable[ulptable.User == user]
