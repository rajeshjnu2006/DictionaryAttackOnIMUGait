...
gen_user_list = ['User' + str(id) for id in range(1, len(gen_user_list) + 1) if 'User' + str(id) not in DefaultParam.BAD_USERS]
ze_imp_user_list = ['User' + str(id) for id in range(1, len(ze_imp_user_list) + 1) if 'User' + str(id) not in DefaultParam.BAD_USERS]
...
ulptable = pd.DataFrame(columns=['user', 'attacktype', 'attacker', 'sensors', 'nfeatures', 'solver', 'cval', 'penalty', 'far', 'frr', 'hter', 'refhter'])
row_counter = 0
########################################################################################################################
...
# gen_user_list = ['User10', 'User12', 'User14']
...
########################################################################################################################
        for num_features in list_num_features:
            ...
            feature_selectors = Prep3.get_feature_matrices(curr_comb, ze_imp_user_list, user, fsselect_method, num_features)
########################################################################################################################
...
########################################################################################################################
            ...
            training_labels = np.concatenate((Prep3.get_labels(comb_selected_sens_gtr, 1), Prep3.get_labels(comb_selected_sens_itr, -1)))
            ...
            # Use the random grid to search for best hyperparameters
            # First create the base model to tune
            CUAuthModel = linear_model.LogisticRegression(random_state=DefaultParam.RANDOM_SEED, tol=1e-5)
...
########################################################################################################################
            ...
            actual_labels_zts = np.concatenate((Prep3.get_labels(comb_selected_sens_gts, 1), Prep3.get_labels(comb_selected_sens_its, -1)))
            ...
            ulptable.loc[row_counter] = [user, "zero-effort", "rest", curr_comb, num_features, solver, cval, penalty,
...
########################################################################################################################
            ...
            for dict_attacker in dict_attacker_list:
                ...
                dict_attack_data = Prep3.get_feature_matrix_for_dict_attack(curr_comb, dict_attacker, feature_selectors, normalizers)
                ...
                actual_labels_dts = np.concatenate((Prep3.get_labels(comb_selected_sens_gts, 1), Prep3.get_labels(dict_attack_data, -1)))
                ...
                row_counter = row_counter + 1
                ulptable.loc[row_counter] = [user, "dict-effort", dict_attacker, curr_comb, num_features, solver, cval,
                                             penalty, tsd_far, tsd_frr, tsd_hter, tsz_hter]
            ...
            if user in DefaultParam.HIGHEFFORT_USER_LIST: # Running only for the first 18 users
                for attempt in attempt_list:
                    # normalized, selected, attack data
                    high_attack_data = Prep3.get_feature_matrix_for_high_attack(curr_comb, high_attacker, attempt, feature_selectors,
                                                                                normalizers)
                    ...
                    # updating the giant perf table
                    row_counter = row_counter + 1
                    ulptable.loc[row_counter] = [user, "high-effort", attempt, curr_comb, num_features, solver,
                                                 cval, penalty, tsh_far, tsh_frr, tsh_hter, tsz_hter]
            print(solver,cval, penalty)
            print(f'{user} | {thebest_dict_attacker} | {the_best_attempt}| {curr_comb} | tr_hter:{tr_hter:.3f} | tsz_frr:{tsz_frr:.3f}, tsz_far:{tsz_far:.3f}, tsz_hter:{tsz_hter:.3f} | max dfar: {max_dfar:.3f} | max_hfar:{max_hfar:.3f}')
