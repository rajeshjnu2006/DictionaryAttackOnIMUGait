...
ulptable = pd.DataFrame(columns=['user', 'attacktype', 'attacker', 'sensors', 'nfeatures', 'cval', 'gamma', 'kernel',
                                 'far', 'frr', 'hter', 'refhter'])
row_counter = 0
########################################################################################################################
...
for user in gen_user_list:
    ...
    for curr_comb in sensor_configurations:
        ...
        for num_features in list_num_features:
            ...
            # First create the base model to tune
            CUAuthModel = svm.SVC(random_state=DefaultParam.RANDOM_SEED, tol= 1e-5)
            SearchTheBestParam = GridSearchCV(estimator=CUAuthModel, param_grid=param_grid, cv=10, n_jobs=6,verbose=0, scoring=scorerHTER)
            SearchTheBestParam.fit(training_data, training_labels)

            cval = SearchTheBestParam.best_params_['C']
            gamma = SearchTheBestParam.best_params_['gamma']
            kernel = SearchTheBestParam.best_params_['kernel']

            ############################################################################################################
            ########### Validation
            ############################################################################################################
            # Retraining the model again and testing under different attack scenarios
            FinalModel = svm.SVC(C=cval, gamma=gamma,kernel=kernel, random_state=DefaultParam.RANDOM_SEED,tol=1e-5)
            ...
            ulptable.loc[row_counter] = [user, "zero-effort", "rest", curr_comb, num_features, cval, gamma, kernel,
            ...
            for dict_attacker in dict_attacker_list:
                ...
                row_counter = row_counter + 1
                ulptable.loc[row_counter] = [user, "dict-effort", dict_attacker, curr_comb, num_features, cval, gamma,
                                             kernel, tsd_far, tsd_frr, tsd_hter, tsz_hter]
            ...
            if user in DefaultParam.HIGHEFFORT_USER_LIST:  # Running only for the first 18 users
                for attempt in attempt_list:
                    ...
                    # updating the giant perf table
                    row_counter = row_counter + 1
                    ulptable.loc[row_counter] = [user, "high-effort", attempt, curr_comb, num_features, cval,
                                                 gamma,kernel, tsh_far, tsh_frr, tsh_hter, tsz_hter]
            print('params:', cval, gamma, kernel)
            print(
                f'{user} | {thebest_dict_attacker} | {the_best_attempt}| {curr_comb} | tr_hter:{tr_hter:.3f} | tsz_frr:{tsz_frr:.3f}, tsz_far:{tsz_far:.3f}, tsz_hter:{tsz_hter:.3f} | max dfar: {max_dfar:.3f} | max_hfar:{max_hfar:.3f}')
